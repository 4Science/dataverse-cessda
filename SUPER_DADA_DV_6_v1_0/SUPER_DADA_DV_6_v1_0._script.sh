...
xpaths=(
    ...
    "/codeBook/stdyDscr/citation/distStmt/distrbtr"
    "/codeBook/stdyDscr/citation/distStmt/distDate"
)
	...
	find /usr/local/payara6/glassfish/domains/domain1/files/ -name "export_ddi.cached" -mtime -0.5 | sed 's/\/export_ddi.cached//g' > $filepath
	#find /usr/local/payara6/glassfish/domains/domain1/files/ -name "export_ddi.cached" | sed 's/\/export_ddi.cached//g' > $filepath
	...
	# As long as i does not equal the number of paths recorded in the index file, operations are carried out once more
		...
		do
			...
			echo "Pull nation to CountrNation.txt"
			...
			echo "I start with transformations"
			...
			for xpath in "${xpaths[@]}"; do
				xmlstarlet ed -O --inplace \
					--insert "$xpath[not(@xml:lang)]" \
					--type attr -n xml:lang -v en \
					export_ddi.cached export_oai_ddi.cached
				
				echo "$xpath xml:lang attribute was successfully added or was already present: $(date)"
			done
			...
			#the previus command delete lang from date, here we add once again
			xmlstarlet ed -O --inplace --insert "/codeBook/stdyDscr/citation/distStmt/distDate[not(@xml:lang)]" --type attr -n xml:lang -v en export_ddi.cached export_oai_ddi.cached

			# Adding an orphan <holdings> element whose @URI attribute receives the dataset's URL as a value

			beginningofurl=https://doi.org
			doi=$(pwd | sudo sed 's/\/usr\|\/local\|\/payara6\|\/glassfish\|\/domains\|\/domain1\|\/files//g')
